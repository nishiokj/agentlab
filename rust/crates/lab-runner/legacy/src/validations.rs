...
pub(crate) fn check_workspace_contract_not_host_path_for_variants(
    variants: &[Variant],
    variant_runtime_profiles: &[VariantRuntimeProfile],
) -> Vec<PreflightCheck> {
    if variants.len() != variant_runtime_profiles.len() {
...
pub(crate) fn check_workspace_contract_not_host_path(
    runtime_profile: &VariantRuntimeProfile,
) -> PreflightCheck {
...
pub(crate) fn check_task_sandbox_bash_plane_for_variants(
    variants: &[Variant],
    variant_runtime_profiles: &[VariantRuntimeProfile],
) -> Vec<PreflightCheck> {
    if variants.len() != variant_runtime_profiles.len() {
...
pub(crate) fn check_task_sandbox_bash_plane(
...
pub(crate) fn check_container_ready(
    ...
    let shell_probe_failures =
        run_bounded_image_probes(&images, "container_ready/shell_probe", |idx, image| {
            ...
            match probe_image_shell(&docker, image) {
                Ok(()) => None,
                Err(err) => Some(format!("{} ({})", image, err)),
            }
        });
...
pub(crate) fn probe_image_shell(
    docker: &crate::backend::docker::DockerRuntime,
    image: &str,
) -> Result<()> {
    let mut spec = crate::backend::docker::ContainerSpec::idle(image.to_string());
    spec.command = vec![
        "/bin/sh".to_string(),
        "-lc".to_string(),
        "exit 0".to_string(),
    ...
    let start_result = docker.start_container(&handle);
    let remove_result = docker.remove_container(&handle, true);
    start_result?;
    remove_result?;
    Ok(())
}

pub(crate) fn check_dependency_files_exist(
    json_value: &Value,
    exp_dir: &Path,
) -> Vec<PreflightCheck> {
...
pub(crate) fn check_workspace_patch_sources_exist(
    variants: &[Variant],
    variant_runtime_profiles: &[VariantRuntimeProfile],
) -> Vec<PreflightCheck> {
