...
Note (2026-02-20): Runner hard-cut to `runtime.agent` with canonical trial output `result.json` (`agent_result_v1`). Any legacy `trial_output.json` / `runtime.agent_loop` references in this draft should be treated as historical.
...
### Trial
One execution of one task under one variant at one replication index. The atomic unit of work. `trial_id = f(variant_id, task_id, repl_idx)`. Runner writes `trial_input.json`, runtime writes `result.json` (agent result contract).
...
### Runtime Command
The user-provided program that executes a trial. Consumes `trial_input.json`, produces `result.json` (`agent_result_v1`). Optional richer integrations can emit hook events/traces.
